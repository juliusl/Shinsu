use lifec::prelude::*;
use shinsu::{NodeExtension, SingleIO, Nodes};
...
fn main() {
    ...
    let mut world = Test::compile_workspace(&workspace, files.iter(), None);
    let node_extension = NodeExtension::new(SingleIO::default());

    node_extension.initialize(&mut world);
    ...
    host.world_mut().exec(|mut nodes: Nodes| {
        nodes.rearrange();
    });
...
impl Project for Test {
    fn interpret(_: &specs::World, _: &Block) {}
}
